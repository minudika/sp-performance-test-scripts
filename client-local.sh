...
download_results() {
    cd ${CLIENT_DIR}/downloaded-results
    mkdir ${SCENARIO}
    ...
    /home/minudika/Projects/WSO2/dev/performance-test/downloaded-results/${SCENARIO}
}

clean_server_1() {
    echo "remove performance results and siddhi apps from worker ${REMOTE_IP1}.."
    sudo ssh -i ${KEY} ${REMOTE_USERNAME1}@${REMOTE_IP1} ./clean.sh
}

clean_server_2() {
    echo "remove performance results and siddhi apps from worker ${REMOTE_IP2}.."
...
while
    status_code=$(curl --write-out %{http_code} --silent --output /dev/null -X GET \
    https://${REMOTE_IP2}:9444/siddhi-apps/TCP_Benchmark/status -H "accept: application/json" -u admin:admin -k)
